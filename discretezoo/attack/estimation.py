...
class DiscreteZOO:
  ...
  Attributes:
    ...
    add_displacement_to_embedding: Boolean flag that determines if we add the
      displacement generated by our samples to the current embeddings. Not doing
      so treats the displacement like a gradient and generates token scores by
      input x gradient.
    discretize_by_cosine: Boolean flag that determines if we discretize the
      displacement by maximum inner-product or by maximum cosine similarity.
  """
  ...
  def __init__(self,
    ...
    Args:
      ...
      add_displacement_to_embedding: Boolean flag that determines if we add the
        displacement generated by our samples to the current embeddings. Not
        doing so treats the displacement like a gradient and generates token
        scores by input x gradient.
      discretize_by_cosine: Boolean flag that determines if we discretize the
        displacement by maximum inner-product or by maximum cosine similarity.
    """
    ...
    self._add_displacement_to_embedding = add_displacement_to_embedding
    self._discretize_by_cosine = discretize_by_cosine

    if self._norm_embeddings:
      self._embeddings = tf.math.l2_normalize(self._embeddings, axis=-1)

  # pylint: disable=missing-function-docstring
  def _embedding_lookup(self, indices: tf.Tensor) -> tf.Tensor:
    return tf.nn.embedding_lookup(self._embeddings, indices)

  def _normalize_by_magnitudes(
      self, displacement_vectors: tf.Tensor,
      displacement_token_similarities: tf.Tensor) -> tf.Tensor:
    """This function normalizes displacement_token_similarities by magnitudes.

    Normalizing the displacement_token_similarities, which is the result of the
    inner-product between displacements and all embeddings, yields the cosine
    similarity between embeddings and displacements.

    Args:
      displacement_vectors: A <float32>[batch_size, embedding_dimension] batch
        of displacement vectors.
      displacement_token_similarities: A <float32>[batch_size, vocab_size]
        tensor containing the result of the matrix multiplication between the
        displacement_vectors and the embeddings.

    Returns:
      The displacement_token_similarities normalized by both the embedding
        magnitudes and the displacement_vectors magnitude, yielding the cosine
        similarities instead of just inner-products.

    """
    # displacement_token_similarities is [batch_size, vocab_size], if we
    # divide it by a [1, vocab_size] vector of embedding magnitudes and a
    # [batch_size, 1] vector of displacement magnitudes, we get the cosine.
    displacement_magnitudes = tf.norm(displacement_vectors,
                                      axis=-1,
                                      keepdims=True)
    if self._norm_embeddings:
      embedding_magnitudes = tf.ones((self._embeddings.shape[0], 1),
                                     dtype=tf.float32)
    else:
      embedding_magnitudes = tf.norm(self._embeddings, axis=-1, keepdims=True)
    # Because both inputs are vectors, this matrix multiplication serves as an
    # outer product.
    normalization_matrix = tf.matmul(displacement_magnitudes,
                                     embedding_magnitudes,
                                     transpose_b=True)
    return displacement_token_similarities / normalization_matrix
  ...
  def _gradient_estimate_and_update(
    ...
    if self._add_displacement_to_embedding:
      # current_embeddings is currently [batch_size, 1, embedding_dim], we need
      # to remove the second dimension to match the reduced_gradient.
      current_embeddings = tf.squeeze(current_embeddings)
      reduced_gradient = current_embeddings + reduced_gradient
